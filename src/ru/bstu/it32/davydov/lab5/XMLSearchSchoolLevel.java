...
class XMLSearchSchoolLevel extends DefaultHandler {
    ...
    protected XMLSearchSchoolLevel() throws RuntimeException, SAXException {
        System.out.print("Название школы: ");
        String school = sin.nextLine();
        searchSchoolName = school.trim();
        byte level;
        while (true) {
            ...
            try {
                searchLevel = Byte.parseByte(str);
            } catch (Exception e) {
                continue;
            }
            if ((searchLevel >= minLevel) && (searchLevel <= maxLevel)) break;
        }
    }
    ...
    public void endElement(String uri, String localName, String qName) throws SAXException {
        if(qName.equals("pupil")) {
            try {
                ...
                if ((searchSchoolName.equals(schoolName)) && (level == searchLevel)) {
                    founded = true;
